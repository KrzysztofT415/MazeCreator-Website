export class Hex {
    ...
    constructor(q, r) {
        ...
        this.center = [q * 30 * 1.5, (r + q / 2.0) * 30 * Math.sqrt(3)]

        this.corners = [...Array(6).keys()].map(v => [this.center[0] + 30 * Math.cos((Math.PI * v) / 3), this.center[1] + 30 * Math.sin((Math.PI * v) / 3)])

        this.edges = [...Array(6).keys()].map(v => [this.corners[v], this.corners[(v + 1) % 6]])

        this.object = document.createElementNS('http://www.w3.org/2000/svg', 'polygon')
        this.object.id = q + '.' + r
        this.object.setAttribute(
            'points',
            this.corners.reduce((acc, v) => acc + ' ' + v.toString())
        )
    ...
    set setState(newState) {
        this.object.classList.replace(``, `status-${newState}`)
    }
